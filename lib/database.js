...
/**
* The windows of the block-time and hash-rate charts on /mining, shortest
* first. A month is thirty days and a year 365, which is what a reader means by
* "the last month" closely enough, and keeps every window an exact number of
* seconds.
*/
var MINING_WINDOWS = [
  {key: '1h', seconds: 3600},
  ...
  {key: '3m', seconds: 90 * 86400},
  {key: '6m', seconds: 180 * 86400},
  {key: '12m', seconds: 365 * 86400}
];
var mining_windows_cache = null, mining_windows_waiting = null;
...
module.exports = {
  ...
  */
  ...
  /**
  * Average block time and hash rate over the MINING_WINDOWS, one hour to one
  * year, all ending now. The definitions are in lib.block_windows.
  *
  * One query for the longest window rather than one per window. A year at
  * ten-minute spacing is some 53 000 rows of three small fields, which is
  * cheaper than eleven round trips, and every window is then cut from the same
  * snapshot, so no two bars can disagree about where the chain stands. A week
  * is fetched beyond the longest window, so the block before it is there even
  * where the chain went quiet for a day, as it did before the fork.
  *
  * Sorted here rather than by the database. A sort on `height` over a range of
  * `time` cannot use the time index for the order, and on the query engine
  * this driver speaks to it would buffer whole block documents against the
  * in-memory sort limit before projecting. The rows are small and already in
  * the process, so sorting them costs a few milliseconds.
  *
  * Cached for a minute, and a miss is fetched once: callers that arrive while
  * the query runs wait for its answer instead of starting their own. The cache
  * lives in each process of the cluster, so the /mining page carries its
  * snapshot into the charts rather than letting them fetch a second one from
  * another worker. A failed query is logged and answered with an empty list
  * for ten seconds, so a database in trouble is not asked again at once.
  */
  get_mining_windows: function(cb) {
    var now = Math.floor(Date.now() / 1000);
    var cached = mining_windows_cache;
    if (cached && now - cached.at < (cached.data.length ? 60 : 10)) { return cb(cached.data); }
    if (mining_windows_waiting) { return mining_windows_waiting.push(cb); }
    mining_windows_waiting = [cb];
    var longest = MINING_WINDOWS[MINING_WINDOWS.length - 1].seconds;
    Block.find({time: {$gte: now - longest - 7 * 86400}}, {height: 1, time: 1, chainwork: 1, _id: 0})
      .lean().exec(function(err, blocks) {
        var data = [];
        if (err) {
          console.log('get_mining_windows: %s', err);
        } else {
          blocks = blocks || [];
          blocks.sort(function(a, b) { return a.height - b.height; });
          data = lib.block_windows(blocks, now, MINING_WINDOWS);
        }
        mining_windows_cache = {at: now, data: data};
        var waiting = mining_windows_waiting;
        mining_windows_waiting = null;
        waiting.forEach(function(f) { f(data); });
      });
  },
