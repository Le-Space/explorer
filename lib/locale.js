...
exports.reloadLocale = function reloadLocale(locale) {
  ...
  for(var i in lsettings)
  {
    ...
    // A locale file may carry keys this module does not declare. Upstream read
    // those as typos: it warned and threw them away. In this fork they are
    // ordinary strings belonging to templates added after the fork, so that
    // branch printed one warning per key on every start -- 39 of them -- while
    // protecting nothing. The check could not protect anything, because the
    // module that actually serves the strings, lib/i18n.js, reads the locale
    // files itself and never consults this list. So an undeclared key is kept.
    //
    // What a locale file must not do is replace this module's own functions,
    // which the discarded branch would have allowed for any key named like one
    // of them, since a function is not `undefined` either.
    if(typeof exports[i] === "function")
    {
      console.warn("Locale key '" + i + "' would replace a function of the locale module and was ignored");
    }
    else
    {
